...
import { NewBedFilter, NewBedList, NewCreateBed } from ".";

class NewBedLayout extends Component {
  state = {
    page: 0,
    limit: 10,
    count: 0,
  ...
  onChange = e => {
    const {page, limit, search_bed_name} = this.state;
    this.setState({
      [e.target.name]: e.target.value
    });
    this.fetchBed(page, limit, "bed" + e.target.value, search_bed_name)
  };
  ...
  render() {
    ...
    const exportData = allBeds.map(bed => {
      const data = {
        BedName: bed.bed_name,
        BedNumber: bed.bed_number,
        BlockName: bed.block_name,
        Variety: bed.variety_name
      };
      return data;
    });
    return (
...
const mapStateToProps = state => ({
  bed: state.bed.bed,
  varietyList: state.scout.allVariety,
